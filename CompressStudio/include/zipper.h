...
namespace cs {
	...
	{
	private:
		static int32_t minizip_add_progress_cb(void* handle, void* userdata, mz_zip_file* file_info, int64_t position);
	public:
		zipper();
		...
		std::vector<std::string> listArchive(const std::string &path, const std::string &pattern = "*") const;
